...
for the DE comparison. We'll also remove genes with very low expression overall
(at least 10 total reads summed across all samples, and at least half of the 
samples overall with nonzero expression).
...
ggplot(exp, aes(x=normcount+1, group=sample, color=condition)) +
  geom_line(stat="density", alpha = 0.5, lwd=1, adjust=0.75) +
  scale_x_continuous(trans="log2") +
  theme_bw() + 
...
ggplot(exp, aes(x=normcount+1, group=interaction(dox, condition), color=condition)) +
  geom_line(stat = "density", lwd = 1, alpha = 0.5, adjust=0.75) +
  scale_x_continuous(trans="log2") +
  theme_bw() + 
  ggtitle("Smoothed density of normalized counts")
...
ggplot(dmrsa %>% filter(delta_mC > 0), 
       aes(x = delta_mC, y = log2FoldChange)) +
  geom_hline(yintercept=0, col="black") +
  geom_hline(yintercept=0, col="white", linetype="dashed") +
  geom_point(size=0.5, alpha=0.75, aes(color = sig)) + 
  theme_bw() + 
...
ggplot(dmrsa, aes(x = stat, y = log2FoldChange)) +
  ...
  geom_label(data =  dmrsa[dmrsa$gene == "SOX2",], col="darkred",
            aes(label="SOX2"), hjust=0.5, vjust=1.25) +
  ggtitle("dmrseq DMRs") +
  geom_vline(xintercept=min(dmrsa$stat[dmrs$qval<0.005]), 
...
### Explore highly significant pairs

```{r}
gtab <- table(dmrsa$gene)
dmrsh <- dmrsa %>%
  mutate(n = unname(gtab[gene])) %>%
 # filter(abs(log2FoldChange) > 1.5) %>%
#  filter(stat > 10) %>%
  dplyr::select(X.seqnames, X.start, X.end, X.gene, X.width, X.L, X.stat,
         X.qval, meanDiff, baseMean, log2FoldChange, padj, index.start, 
         index.end, Control, Methylated) %>%
  mutate(direction = ifelse(log2FoldChange > 0, "Positive", "Negative"),
         subjectHits = 1:n(),
         sig = padj < 0.05 & X.qval < 0.10)

ggplot(dmrsh, aes(x= baseMean, y =log2FoldChange, color = sig)) + 
...
ggplot(dmrsh, aes(x= Control+1, y =log2FoldChange, color = sig)) + 
  geom_point(alpha = 0.5 ) +
  scale_x_continuous(trans ="log10") +
  scale_color_manual(values = c("black", "red")) +
  theme_bw() +
  xlab("Expression in control group")

indexRanges <- IRanges(dmrsh$index.start, dmrsh$index.end)
prop.mat.dmr <- extractROWS(getMeth(bs, type="raw"), indexRanges)
prop.C <- IRanges::relist(prop.mat.dmr[,pData(bs)$condition=="Control"], indexRanges)
prop.M <- IRanges::relist(prop.mat.dmr[,pData(bs)$condition=="Methylated"], indexRanges)
varC <- unlist(lapply(prop.C, function(x) 
               max(DelayedMatrixStats::colSds(x, na.rm=TRUE))))
varM <- unlist(lapply(prop.M, function(x) 
               max(DelayedMatrixStats::colSds(x, na.rm=TRUE))))
dmrsh <- dmrsh %>%
  mutate(varC = varC, varM = varM)

ggplot(dmrsh, aes(x = varC, y = varM, color = sig)) +
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("black", "red")) +
  theme_bw()

ggplot(dmrsh, aes(x= varC, y =log2FoldChange, color = sig)) + 
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("black", "red")) +
  xlab("Intra-sample variation of control samples") +
  theme_bw()

ggplot(dmrsh, aes(x= varM, y =log2FoldChange, color = sig)) + 
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("black", "red")) +
  xlab("Intra-sample variation of treated samples") +
  theme_bw()

ggplot(dmrsh, aes(x= varM+varC, y =log2FoldChange, color = sig)) + 
  geom_point(alpha = 0.5) +
  scale_color_manual(values = c("black", "red")) +
  xlab("Intra-sample variation of both sample types") +
  theme_bw()

table(dmrsh$log2FoldChange > 0, dmrsh$sig)
