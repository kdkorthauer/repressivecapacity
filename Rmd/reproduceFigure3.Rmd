...
sqlfile <- file.path(datdir,
                    paste(Sys.Date(), "SRAmetadb.sqlite.gz", sep= '.'))
...
if(length(bam.files) > 0){
  align(index = indexfile, 
      readfile1 = file.path(fqdir, all.fastq), 
      TH1 = 2, type = 1,
      input_format = "FASTQ", 
...
for(bf in bedfiles){
  if (!file.exists(bf)){
    system(paste0("macs2 callpeaks -t ", gsub("_peaks.bed", ".bam", bf), 
...
for (bf in bedfiles[grepl("H3K4", bedfiles)]){
  ...
  peaks <- c(peaks,
             makeGRangesFromDataFrame(read_tsv(bf, 
                    colnames = c("chr", "start", "end", "length",
                                 "summit", "pileup", "foldenrichment"))))
}
peaks <- reduce(peaks)
...
totalReads <- colSums(assays(summarizeOverlaps(peaks, bfL, mode = "Union"))[[1]])
...
```{r, intersectchip}
rse <- summarizeOverlaps(makeGRangesFromDataFrame(ts4), bfL, mode = "Union")
counts <- apply(assays(rse)[[1]], 1, function(x) x / sizeFactors)

# add counts to UMR dataframe
ts4 <- ts4 %>% 
  mutate(sig = fdr_RNAseq < 0.05,
         chip.dox = counts[,grepl("Meth", colnames(counts))],
         chip.ctrl = counts[,grepl("Ctrl", colnames(counts))],
