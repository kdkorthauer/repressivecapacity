...
plotOneFrame <- function(dmrtab, cutoff, on_thresh=50){
  ...
  A <- ggplot(dmrs, aes(x = stat, y = log2FoldChange)) +
    ...
    geom_label(data =  dmrs[dmrs$gene == "SOX2",], col=sox2col,
  ...
  B <- ggplot(dmrs %>% filter(stat > cutoff),
   ...
   ylab("Density") +
   ggtitle("Expression in significant DMRs") +
   labs(fill="Direction \nof effect") +
   xlim(-3,3) +
   theme(legend.position = "none") + 
   ylim(0,1.8)  +
   draw_label("Activated", 2.2, 1.75, size = 12) +
   draw_label("Repressed", -2, 1.75, size = 12)
