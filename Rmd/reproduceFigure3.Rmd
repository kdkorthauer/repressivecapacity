...
# don't need to extract the methylation estimates since already have WGBS data
...
In the manuscript, the normalization of H3K4me3 ChIP-BS data is described
as follows: 
...
treatment and control are less than 0.2. We'll also exclude regions that overlap 
with a ZF binding site. Then we'll count up the number of reads in each
...
Figure 3 is used make some pretty bold claims. For example, the manuscript states that
...
## Reproduce Figure 3C for H3K4me3

We first start with Figure 3C, the scatterplot of methylation difference and
difference in H3K4me3. 

First, we need to find overlaps between the UMRs and H3K4me3 reads. We also follow
Ford et al. and expand the UMR regions to a 5kb window centered at the UMRs.

They also excluded regions with less than 2000 total reads mapping. That absolute 
cutoff removes a high proportion of the data (more than 85%); perhaps due to differences
in mapping procedure, or how the normalization scaling was carried out. We'll use
a cutoff of 50 total (normalized) reads

```{r, intersectchip}
umrs <- makeGRangesFromDataFrame(ts3)
md <- meanDiff.dss(bs, dmrs=umrs, testCovariate="condition")
# uniform sizes of umrs
umrs <- resize(umrs, width = 5000, fix = "center")
seqlevels(umrs) <- gsub("chr", "", seqlevels(umrs))
rse <- summarizeOverlaps(umrs, bfL.H3K4, mode = "Union")
...
ts3.h <- ts3 %>% 
  mutate(chip.dox.norm = assays(rse)$normcounts[,grepl("Meth", colnames(rse))],
         ...
         delta_mC_orig = methylation_level_in_ZF_D3A_plus_dox - methylation_level_in_MCF7_control,
         delta_mC = md,
...
```{r, fig3c}
# raw read counts
ggplot(ts3.h %>% filter(delta_mC > 0), aes(y = FC, x = delta_mC)) +
  ...
  ggtitle("Raw ChIP-BS read counts") +
  geom_smooth()

# normalized
ggplot(ts3.h %>% filter(delta_mC > 0), aes(y = FC.norm, x = delta_mC)) +
  geom_point(size=0.5, alpha=0.75) + 
  geom_hline(yintercept=1, col="black") +
  geom_hline(yintercept=1, col="white", linetype="dashed") +
  scale_y_continuous(trans="log2") +
...
has much more variability in the spread of the points (here we are plotting the 
log2 ratio on the y-axis instead of the absolute difference). The downward trend 
in H3K4me3 decrease for higher values of methylation increase is much more apparent
in this this plot.

## Create Figure 3C for RNA PolII 

This figure was not included in the paper, but we'll look at it here.

First, we need to find overlaps between the UMRs and H3K4me3 reads. In the paper,
they used a 5kb window centered at the UMRs, but that seems to dilute the signal
so we won't do that. 

Since the average read counts in these regions are much lower than for the 
H3KM4 data, we will use a lower threshold for the total number of reads across
dox and control (50 instead of 500).

```{r, intersectchip-polII}
umrs <- makeGRangesFromDataFrame(ts3)
# uniform sizes of umrs
umrs <- resize(umrs, width = 5000, fix = "center")
seqlevels(umrs) <- gsub("chr", "", seqlevels(umrs))
rse <- summarizeOverlaps(umrs, bfL.PolII, mode = "Union")
assays(rse)$normcounts <- t(apply(assays(rse)$counts, 1,
...
ts3.p <- ts3 %>% 
  mutate(chip.dox.norm = assays(rse)$normcounts[,grepl("Meth", colnames(rse))],
         ...
         FC.norm = chip.dox.norm / chip.ctrl.norm,
         FC = chip.dox / chip.ctrl) %>%
  filter(chip.dox.norm + chip.ctrl.norm > 10)
```
...
```{r, fig3c-pollII}
# raw read counts
ggplot(ts3.p %>% filter(delta_mC > 0), 
       aes(y = FC, x = delta_mC)) +
  geom_point(size=0.5, alpha=0.75) + 
  scale_y_continuous(trans="log2") +
  geom_hline(yintercept=1, col="black") +
...
# Figure 3C for DMRs instead of UMRs

We also wish to redo Fig 3 with DMRs from dmrseq, instead of UMRs as done in the paper.

## Reproduce Figure 3C for H3K4me3 data
...
## Remake Figure 3C for DNA PolII data
...
To construct these figures, we need to intersect the ChIP-BS data with the UMRs. 
For now, we'll
use the methylation measurements from WGBS data (even though using the ChIP-BS
data would be more direct, this should still show a signal). See the 
`methylationCountAnalysis.Rmd` for details on how the WGBS data is downloaded and 
processed.

Here we are interested in merged peaks between control and dox (using peak 
...
peaks.all <- GRangesList(peaks.all[[1]][ol@from], 
                         peaks.all[[2]][ol@to])
peaks.common <- peaks.all[[1]]
start(peaks.common) <- pmax(start(peaks.all[[1]]), start(peaks.all[[2]]))
end(peaks.common) <- pmin(end(peaks.all[[1]]), end(peaks.all[[2]]))

# intersect the UMRs with the peaks
seqlevels(peaks.common) <- paste0("chr", seqlevels(peaks.common))
ol <- findOverlaps(peaks.common, makeGRangesFromDataFrame(ts3))
int <- peaks.common[unique(ol@from), ]

# intersect the peak-UMRs with chipbs
int.chip <- bs.h3k4[findOverlaps(bs.h3k4, int)@from, ]
int.chip <- int.chip[findOverlaps(int.chip, makeGRangesFromDataFrame(ts3))@from, ]
int.wgbs <- bs[findOverlaps(bs, int)@from, ]
int.wgbs <- int.wgbs[findOverlaps(int.wgbs, makeGRangesFromDataFrame(ts3))@from, ]

# filter both by min coverage of 5 in each condition
cov.mat <- getCoverage(int.chip, type="Cov")
filter <- DelayedMatrixStats::rowSums2(cov.mat > 10) < 2
filter <- which(filter)
int.chip <- int.chip[-filter,]

cov.mat <- getCoverage(int.wgbs, type="Cov")
filter <- pmax( 1*(DelayedMatrixStats::rowSums2(cov.mat[,pData(bs)$condition == "Control"]) < 10),
                1*(DelayedMatrixStats::rowSums2(cov.mat[,pData(bs)$condition == "Methylated"]) < 10))
filter <- which(filter > 0)
...
prop1 <- as.data.frame(getMeth(int.chip, type="raw")) %>% 
  mutate(region = findOverlaps(int.chip, makeGRangesFromDataFrame(ts3))@to) %>%
...
prop2 <-  (as.data.frame(getMeth(int.wgbs, type="raw")) %>% 
    mutate(region = findOverlaps(int.wgbs, makeGRangesFromDataFrame(ts3))@to) %>%
...
# plot
ggplot(prop, aes(x = type, y = Meth, fill = Group)) +
  scale_fill_manual(values = c("red", "blue")) +
  geom_boxplot(outlier.shape=NA) + 
  theme_classic() + 
  xlab("Experiment type") +
  ylab("Average methylation proportion") +
  ggtitle("UMR : H3K4me3 Peak intersections")
```

Note that `r signif(100*length(unique(prop1$region))/nrow(ts3), 3)`%
of UMRs overlap with an H3K4Me3 peak. We'll look at how the proportion of 
overlapping UMRs changes with methylation effect size.

```{r, fig.width = 10}
peaks.chip <- bs.h3k4[findOverlaps(bs.h3k4, peaks.common)@from, ]

# filter both by min coverage of 5 in each condition
cov.mat <- getCoverage(peaks.chip, type="Cov")
filter <- DelayedMatrixStats::rowSums2(cov.mat > 10) < 2
filter <- which(filter)
peaks.chip <- peaks.chip[-filter,]

ol <- findOverlaps(peaks.chip, makeGRangesFromDataFrame(ts3))
ol2 <- findOverlaps(peaks.chip, peaks.common)
ol3 <- findOverlaps(peaks.common, makeGRangesFromDataFrame(ts3))

prop.peaks <- as.data.frame(getMeth(peaks.chip, type="raw")) 

prop.peaks$region <- NA
prop.peaks$region[ol@from] <- ol@to

prop.peaks <- prop.peaks %>%
  mutate(peak = ol2@to) %>% 
  mutate(delta_mC = DOX - noDOX) %>%
  group_by(peak) %>%
  summarize(regions = length(unique(na.omit(region))),
            delta_mC = mean(delta_mC, na.rm=TRUE)) %>% 
  mutate(bins = cut(delta_mC, 
                    breaks = c(min(delta_mC)-0.01,seq(0,max(delta_mC)+0.05,by=0.05)), 
                    dig.lab = 2)) %>%
  group_by(bins) %>%
  summarize(nOverlap = sum(regions > 0),
            n = n()) %>%
  mutate(pOverlap = nOverlap / n)

ggplot(prop.peaks, aes(x = bins, y = pOverlap)) +
  geom_point() +
  geom_line(aes(group = 1)) +
  xlab(expression(paste(Delta, "mCG bin"))) +  
  ylab("Proportion of UMRs overlapping H3K4me3 peaks")
```

## Figure 3D (RNA Pol II)

We'll do the same as in the previous section, but for RNA Pol II data.

```{r, fig5b-pol, fig.height=4, fig.width=7}
# read in called peaks from macs
bedfiles <- file.path(datdir, paste0(dataset,"_", grouping, "_peaks.xls")) 
peaks.all <- NULL
for (bf in bedfiles[grepl("Pol", bedfiles)]){
  # import peak bed files as GRanges
  peaks <- read_tsv(bf, comment = "#", skip = 1, guess_max = 5e5) %>%
                                mutate(condition = basename(bf))
  if (length(peaks.all) > 0){
...
# intersect the peak-UMRs with chipbs
int.chip <- bs.pol[findOverlaps(bs.pol, int)@from, ]
int.chip <- int.chip[findOverlaps(int.chip, makeGRangesFromDataFrame(ts3))@from, ]
int.wgbs <- bs[findOverlaps(bs, int)@from, ]
int.wgbs <- int.wgbs[findOverlaps(int.wgbs, makeGRangesFromDataFrame(ts3))@from, ]
...
ggplot(prop, aes(x = type, y = Meth, fill = Group)) +
  ...
  xlab("Experiment type") +
  ylab("Average methylation proportion") +
  ggtitle("UMR : RNA PolII Peak intersections")
```

Note that `r signif(100*length(unique(prop1$region))/nrow(ts3), 3)`%
of UMRs overlap with an RNA PolII peak. We'll look at how the proportion of 
overlapping UMRs changes with methylation effect size.

```{r, fig.width = 10}
peaks.chip <- bs.pol[findOverlaps(bs.pol, peaks.common)@from, ]
...
ggplot(prop.peaks, aes(x = bins, y = pOverlap)) +
  ...
  geom_line(aes(group = 1)) +
  xlab(expression(paste(Delta, "mCG bin"))) +  
  ylab("Proportion of UMRs overlapping RNA PolII peaks")
```

# Figure 3D and E with DMRs instead of UMRS

We repeat the past couple of sections using dmrseq DMRs instead of UMRs.
...
```{r, fig5b-dmrs, fig.height=4, fig.width=7}
dmrs <- (dmrs <- readRDS(file.path(resdir, "regions.rds")))[dmrs$qval < 0.10, ]
...
# intersect the UMRs with the peaks
seqlevels(dmrs) <- gsub("chr", "", seqlevels(dmrs))
ol <- findOverlaps(peaks.common, dmrs)
int <- peaks.common[unique(ol@from), ]

# intersect the peak-UMRs with chipbs
seqlevels(int) <- paste0("chr", seqlevels(int))
seqlevels(dmrs) <- paste0("chr", seqlevels(dmrs))
int.chip <- bs.h3k4[findOverlaps(bs.h3k4, int)@from, ]
int.chip <- int.chip[findOverlaps(int.chip, dmrs)@from, ]
int.wgbs <- bs[findOverlaps(bs, int)@from, ]
int.wgbs <- int.wgbs[findOverlaps(int.wgbs, dmrs)@from, ]
...
# compute methylation proportions
prop1 <- as.data.frame(getMeth(int.chip, type="raw")) %>% 
  mutate(region = findOverlaps(int.chip, dmrs)@to) %>%
  gather("Group", "Meth", 1:2) %>%
  mutate(type = "ChIP-purified") %>% 
  group_by(region, Group, type) %>%
  summarize(Meth = mean(Meth, na.rm = TRUE))
...
prop2 <-  (as.data.frame(getMeth(int.wgbs, type="raw")) %>% 
    mutate(region = findOverlaps(int.wgbs, dmrs)@to) %>%
    gather("Sample", "Meth", 1:6) %>%
    mutate(Group = ifelse(grepl("Vector", Sample) | grepl("noDox", Sample),
                          "noDOX", "DOX"),
           type = "WGBS")) %>% 
...
ggplot(prop, aes(x = type, y = Meth, fill = Group)) +
  ...
  xlab("Experiment type") +
  ylab("Average methylation proportion") +
  ggtitle("DMR : H3K4me3 Peak intersections")
```

Note that `r signif(100*length(unique(prop1$region))/length(dmrs), 3)`%
of DMRs overlap with an H3K4Me3 peak. We'll look at how the proportion of 
overlapping UMRs changes with methylation effect size.
...
ol <- findOverlaps(peaks.chip, dmrs)
ol2 <- findOverlaps(peaks.chip, peaks.common)
...
ggplot(prop.peaks, aes(x = bins, y = pOverlap)) +
  ...
  geom_line(aes(group = 1)) +
  xlab(expression(paste(Delta, "mCG bin"))) +  
  ylab("Proportion of DMRs overlapping H3K4me3 peaks")
```
...
```{r, fig5b-pol-dmrs, fig.height=4, fig.width=7}
dmrs <- (dmrs <- readRDS(file.path(resdir, "regions.rds")))[dmrs$qval < 0.10, ]
...
ggplot(prop, aes(x = type, y = Meth, fill = Group)) +
  ...
  xlab("Experiment type") +
  ylab("Average methylation proportion") +
  ggtitle("DMR : RNA PolII Peak intersections")
```
...
ggplot(prop.peaks, aes(x = bins, y = pOverlap)) +
  ...
  geom_line(aes(group = 1)) +
  xlab(expression(paste(Delta, "mCG bin"))) +  
  ylab("Proportion of DMRs overlapping RNA PolII peaks")
```
