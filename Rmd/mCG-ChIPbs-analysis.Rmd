...
#load bsseq data (processed in mCG-RNAseq-analysis.Rmd)
...
The DMRs were prepared in the `mCG-RNAseq-analysis.Rmd` analysis.
...
se_lo <- sqrt(1/dn + 1/up)*qnorm(0.975)
lodds.ci <- log(odds) + c(-1,1)*se_lo
odds.ci <- exp(lodds.ci)
prop.ci <- binom.confint(dn, n, method="wilson", conf.level = 0.95)
...
data. See the 
`mCG-RNAseq-analysis.Rmd` for details on how the WGBS data is downloaded and 
processed.
