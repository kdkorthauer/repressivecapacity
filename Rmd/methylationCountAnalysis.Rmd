...
were at least 20kb away from a ZF binding site or putative DMR (by dmrseq, with
FDR level 0.25).
...
dmr_genes <- unique(c(annot$symbol[ol@to[mcols(ol)$distance <= 20000]],
                      annot$symbol[ol2@to]))
dmr_genes <- dmr_genes[!is.na(dmr_genes)]

zf_genes <- (ts1 %>%
  filter(distance_to_nearest_promoter <= 20000))$closest_gene_promoter
