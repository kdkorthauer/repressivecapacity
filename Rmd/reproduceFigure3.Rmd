...
is the methylation level in UMRs that overlap a peak with UMRs that do not overlap 
...
First we'll make the plot comparing average methylation values in UMRs by whether
...
# subset on normalized read count in common peaks
rse <- summarizeOverlaps(peaks.common, bfL.H3K4, mode = "Union")
assays(rse)$normcounts <- t(apply(assays(rse)$counts, 1,
                                 function(x) x / sizeFactors.H3K4))
int <- peaks.common[which(rowSums(assays(rse)$normcounts > 100) == 2)]
seqlevels(int) <- paste0("chr", seqlevels(int))

# intersect the peak-UMRs with chipbs
int.chip <- bs.h3k4[findOverlaps(bs.h3k4, makeGRangesFromDataFrame(ts3))@from, ]
int.wgbs <- bs[findOverlaps(bs, makeGRangesFromDataFrame(ts3))@from, ]

# filter both by min coverage of 5 in each condition
cov.mat <- getCoverage(int.chip, type="Cov")
filter <- DelayedMatrixStats::rowSums2(cov.mat > 4) < 2
filter <- which(filter)
...
prop1 <- as.data.frame(getMeth(int.chip, type="raw")) %>% 
  mutate(region = findOverlaps(int.chip, makeGRangesFromDataFrame(ts3), select = "first"),
         status = ifelse(countOverlaps(int.chip, int) > 0, "H3K4me3 Peak", "No Peak")) %>%
...
prop2 <-  (as.data.frame(getMeth(int.wgbs, type="raw")) %>% 
    mutate(region = findOverlaps(int.wgbs, makeGRangesFromDataFrame(ts3), select = "first"),
           status = ifelse(countOverlaps(int.wgbs, int) > 0, "H3K4me3 Peak", "No Peak")) %>%
...
# plot
ggplot(prop, aes(x = Group, y = Meth, color =  status)) +
  ...
  labs(color = "Overlap") +
  ggtitle("UMRs and H3K4me3 Peaks")
```

Note that `r signif(100*length(unique(prop1$region[prop1$status == "H3K4me3 Peak"]))/nrow(ts3), 3)`%
of UMRs overlap with an H3K4Me3 peak. 
...
assays(rse)$normcounts <- t(apply(assays(rse)$counts, 1,
                                 function(x) x / sizeFactors.PolII))
int <- peaks.common[which(rowSums(assays(rse)$normcounts > 50) == 2)]
seqlevels(int) <- paste0("chr", seqlevels(int))

# intersect the peak-UMRs with chipbs
int.chip <- bs.pol[findOverlaps(bs.pol, makeGRangesFromDataFrame(ts3))@from, ]
int.wgbs <- bs[findOverlaps(bs, makeGRangesFromDataFrame(ts3))@from, ]
...
prop1 <- as.data.frame(getMeth(int.chip, type="raw")) %>% 
  mutate(region = findOverlaps(int.chip, makeGRangesFromDataFrame(ts3), select = "first"),
         status = ifelse(countOverlaps(int.chip, int) > 0, "PolII Peak", "No Peak")) %>%
  gather("Group", "Meth", 1:2) %>%
  mutate(type = "ChIP-purified") %>% 
  group_by(region, Group, type, status) %>%
  summarize(Meth = mean(Meth, na.rm = TRUE)) 
...
prop2 <-  (as.data.frame(getMeth(int.wgbs, type="raw")) %>% 
    mutate(region = findOverlaps(int.wgbs, makeGRangesFromDataFrame(ts3), select = "first"),
           status = ifelse(countOverlaps(int.wgbs, int) > 0, "PolII Peak", "No Peak")) %>%
    gather("Sample", "Meth", 1:6) %>%
    mutate(Group = ifelse(grepl("Vector", Sample) | grepl("noDox", Sample),
                          "noDOX", "DOX"),
           type = "WGBS")) %>% 
...
ggplot(prop, aes(x = Group, y = Meth, color =  status)) +
  ...
  ylab("Average methylation proportion") +
  labs(color = "Overlap") +
  ggtitle("UMRs and RNA PolII Peaks")
```

Note that `r signif(100*length(unique(prop1$region[prop1$status == "PolII Peak"]))/nrow(ts3), 3)`%
of UMRs overlap with an RNA PolII peak.
...
# intersect the peak-UMRs with chipbs
int.chip <- bs.h3k4[findOverlaps(bs.h3k4, dmrs)@from, ]
int.wgbs <- bs[findOverlaps(bs, dmrs)@from, ]
...
# compute methylation proportions
prop1 <- as.data.frame(getMeth(int.chip, type="raw")) %>% 
  mutate(region = findOverlaps(int.chip, dmrs, select = "first"),
         status = ifelse(countOverlaps(int.chip, int) > 0, "H3K4me3 Peak", "No Peak")) %>%
  gather("Group", "Meth", 1:2) %>%
  mutate(type = "ChIP-purified") %>% 
  group_by(region, Group, type, status) %>%
...
prop2 <-  (as.data.frame(getMeth(int.wgbs, type="raw")) %>% 
    mutate(region = findOverlaps(int.wgbs, dmrs, select = "first"),
           status = ifelse(countOverlaps(int.wgbs, int) > 0, "H3K4me3 Peak", "No Peak")) %>%
    gather("Sample", "Meth", 1:6) %>%
    mutate(Group = ifelse(grepl("Vector", Sample) | grepl("noDox", Sample),
                          "noDOX", "DOX"),
...
ggplot(prop, aes(x = Group, y = Meth, color =  status)) +
  ...
  ylab("Average methylation proportion") +
  labs(color = "Overlap") +
  ggtitle("DMRs and H3K4me3 Peaks")
```

Note that `r signif(100*length(unique(prop1$region))/length(dmrs), 3)`%
of DMRs overlap with an H3K4Me3 peak. 
...
# intersect the peak-UMRs with chipbs
int.chip <- bs.pol[findOverlaps(bs.pol, dmrs)@from, ]
int.wgbs <- bs[findOverlaps(bs, dmrs)@from, ]
...
ggplot(prop, aes(x = Group, y = Meth, color =  status)) +
  ...
  ylab("Average methylation proportion") +
  labs(color = "Overlap") +
  ggtitle("DMRs and RNA PolII Peaks")
```
