...
ggplot(exp, aes(x=normcount+1, group=sample, color=condition)) +
  geom_line(stat="density", alpha = 0.5, lwd=1, adjust=0.75, aes(linetype=dox)) +
  scale_x_continuous(trans="log2") +
  theme_bw() + 
...
ggplot(exp, aes(x=normcount+1, group=interaction(dox, condition), color=condition)) +
  geom_line(stat = "density", lwd = 1, alpha = 0.5, adjust=0.75, aes(linetype=dox)) +
  scale_x_continuous(trans="log2") +
  theme_bw() + 
  ggtitle("Smoothed density of normalized counts")
...
dmrsh <- dmrsa %>%
  ...
  dplyr::select(X.seqnames, X.start, X.end, X.gene, X.width, X.L, X.stat,
         X.qval, meanDiff, baseMean, log2FoldChange, padj) %>%
  mutate(direction = ifelse(log2FoldChange > 0, "Positive", "Negative"),
         subjectHits = 1:n())

ggplot(dmrsh, aes(x= baseMean, y =log2FoldChange)) + 
  geom_point() +
  scale_x_continuous(trans ="log10") +
  theme_bw()
  
ol <- findOverlaps(bs, makeGRangesFromDataFrame(dmrsh))
mm <- getMeth(bs[ol@from,], type="raw")
sds <- rowSds(mm, na.rm=TRUE)
intermed <- apply(mm, 1, function(x) sum(x > 0.25 | x < 0.75)/length(x))

ol <- data.frame(ol) %>%
  mutate(sds = sds) %>%
  group_by(subjectHits) %>%
  summarize(mean_sd = mean(sds))
dmrsh <- left_join(dmrsh, ol, by = "subjectHits")

ggplot(dmrsh, aes(x= mean_sd, y =log2FoldChange)) + 
  geom_point() +
  scale_x_continuous(trans ="log10") +
  theme_bw()

table(dmrsh$log2FoldChange > 0)
