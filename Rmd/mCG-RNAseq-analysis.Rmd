...
pdf("../plots/figS3a.pdf", width = 7, height = 3)
...
pdf("../plots/figS3b.pdf", width = 7, height = 3)
...
A2 <- ggplot(dmrsa %>% filter(CGI), aes(x = stat, y = log2FoldChange)) +
  ...
  ggtitle("dmrseq DMRs") +
  ylim(-3,3) + 
  geom_vline(xintercept=min(dmrsa$stat[dmrsa$qval<fdrlevel.dmr.highconf & dmrsa$CGI]), 
             linetype="dashed", color="grey20")
A2 
...
prop.ci <- binom.confint(de_dn, n, method="wilson", conf.level = 0.95)
...
lodds.ci <- log(odds) + c(-1,1)*se_lo
odds.ci <- exp(lodds.ci)
prop.ci <- binom.confint(de, n, method="wilson", conf.level = 0.95)
...
```{r}
calc_odds <- function(dmrs, fdr.dmr, fdr.de, x.on, top.x, CGI = NULL, level = 0.95){
    ...
    ci <- binom.confint(sum(dmrs.filt$log2FoldChange < 0 & dmrs.filt$padj < fdr.de),
                           p.n, method="wilson", conf.level = level)
    prop.ci <- c(max(ci$lower, 0), min(ci$upper, 1))
    prop <- ci$mean
...
```{r}
xon <- c(0,50,250)
dmr.fdr <- c(0.005, 0.01, 0.025, 0.05, 0.10)
de.fdr <- c(1e-5, 0.01, 0.05, 0.10, 0.25, 0.5, 0.75, 1)

df <- df.cgi <- df.non <- data.frame(matrix(ncol = 12, 
...
for (x in xon){
  for (y in dmr.fdr){
    for (z in de.fdr){
      if(y == min(dmr.fdr)){
        df[ct, 1:3] <- c(y, z, x)
        df[ct, 4:12] <- unlist(calc_odds(dmrs, y, z, x, top.x=100, CGI = NULL, level = 0.95))
      
        df.cgi[ct, 1:3] <- c(y, z, x)
        df.cgi[ct, 4:12] <- unlist(calc_odds(dmrs, y, z, x, top.x=100, CGI = TRUE, level = 0.95))
      
        df.non[ct, 1:3] <- c(y, z, x)
        df.non[ct, 4:12] <- unlist(calc_odds(dmrs, y, z, x, top.x=100, CGI = FALSE, level = 0.95))
        ct <- ct + 1
      }
      
      df[ct, 1:3] <- c(y, z, x)
      df[ct, 4:12] <- unlist(calc_odds(dmrs, y, z, x, top.x=Inf, CGI = NULL, level = 0.95))
      
      df.cgi[ct, 1:3] <- c(y, z, x)
      df.cgi[ct, 4:12] <- unlist(calc_odds(dmrs, y, z, x, top.x=Inf, CGI = TRUE, level = 0.95))
      
      df.non[ct, 1:3] <- c(y, z, x)
      df.non[ct, 4:12] <- unlist(calc_odds(dmrs, y, z, x, top.x=Inf, CGI = FALSE, level = 0.95))
      ct <- ct + 1
    }
  }
}
...
```{r}
ggplot(df.cgi %>% filter(count.on %in% c(50)) %>% filter(o.n != 100),
         aes(x = fdr.de, y = odds, group = fdr.dmr, color = fdr.dmr, 
             fill = fdr.dmr)) +
    scale_y_continuous(trans= "log2") +
    geom_hline(yintercept = 1, linetype = "dashed", color = "grey") +
