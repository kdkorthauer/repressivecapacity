...
We first generate an index reference genome, and then use Rsubread to perform the
mapping. 

```{r, processchip}
# build index for subread
indexfile <- file.path(fqdir, "hg19index")
if (!file.exists(paste0(indexfile, ".reads"))){ 
  buildindex(basename = indexfile,
           reference = assemblyfile,
           memory=60000)
}

# map with Rsubread
bam.files <- paste0(dataset,"_", grouping, ".bam")
done <- list.files(fqdir, pattern="*.bam")
if (length(done)>0){
 done <- done[file.info(file.path(fqdir,done))$size > 0]
 bam.files <- bam.files[!(bam.files %in% done)]
}

all.fastq <- file.path(fqdir, gsub(".fastq", "_trimmed.fq",  
                                   paste0(all.sra, ".fastq")))
if(length(bam.files) > 0){
  align(index = indexfile, 
      readfile1 = all.fastq, 
      TH1 = 2, type = 1,
      input_format = "FASTQ", 
      output_file = file.path(fqdir, bam.files),
      nthreads = ncores)
}

# build index for bowtie
indexDir <- file.path(fqdir)
indexFile <- "hg19index_bowtie"
if (!file.exists(file.path(indexDir, paste0(indexFile, ".rev.1.ebwt")))){ 
  bowtie_build(references = assemblyfile, outdir = indexDir, 
                    prefix = indexFile, force=TRUE)
}

# align with bowtie
all.fastq <- file.path(fqdir, gsub(".fastq", "_trimmed.fq", 
                                   paste0(all.sra, ".fastq")))
bam.files <- paste0(dataset,"_", grouping, "_bowtie.bam")

for (j in seq_along(bam.files)){
  bf <- bam.files[j]
  if (!file.exists(file.path(fqdir, gsub("bam", "sam", bf)))){
    message("Mapping ", bf, " with bowtie...")
    bowtie(sequences = all.fastq[j], 
       index = file.path(indexDir, "hg19index_bowtie"), 
       outfile = file.path(fqdir, gsub(".bam", ".sam", bf)), 
       sam = TRUE, best = TRUE, force = TRUE,
       chunkmbs = 2000,
       threads = ncores)
  }else{
    message(bf, " already mapped with bowtie.")
  }
    
  if(!file.exists(file.path(fqdir, bf))){
    message("Converting ", bf, " to bam format")
    system(paste0("samtools view -Sb ", file.path(fqdir, gsub(".bam", ".sam", bf)),
                  " -@ ", ncores,
                  " > ", file.path(fqdir, bf)))
  }else{
    message(bf, "already converted to bam format")
  }
}

# sort bamfiles
bam.files <- file.path(fqdir, paste0(dataset,"_", grouping, "_bowtie.bam"))
for (bf in bam.files){
  if (!file.exists(gsub("bowtie.bam", "bowtie_sorted.bam", bf))){
    message("Sorting bam file: ", bf)
    system(paste0("samtools sort ", bf, " -@ ", ncores, " -o ", 
                  gsub("bowtie.bam", "bowtie_sorted.bam", bf)))
  }else{
...
bam.files <- file.path(fqdir, paste0(dataset,"_", grouping, "_bowtie_sorted.bam"))
...
bam.files <- file.path(fqdir, paste0(dataset,"_", grouping, "_bowtie_sorted.bam"))
logfile <- file.path(fqdir, "mappingstats_bowtie.rds")
...
for(bf in bedfiles){
  if (!file.exists(bf)){
    system(paste0("macs2 callpeak -t ", 
                  file.path(fqdir, basename(gsub("_peaks.xls", ".bam", bf))), 
                  " -g 2.7e9 --outdir ", datdir, " -n ", 
                  basename(gsub("_peaks.xls", "", bf))))
  }
}
