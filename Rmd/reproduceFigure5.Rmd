...
Code to try and reproduce figure 5 from the Lister Lab paper by Ford et al., 
...
With the tables in hand, we attempt to reproduce Figure 5 from the manuscript.
...
```{r, extraplot2, eval=FALSE, echo = FALSE}
...
```{r, extraplots1, eval=FALSE, echo=FALSE}
B1 <- ggplot(ts4c , #%>% filter(sig==TRUE), 
...
B1 <- ggplot(ts4c , #%>% filter(sig==TRUE), 
             aes(log2FC_RNAseq)) +
  geom_histogram(breaks=seq(-3,3, by=1/5), aes(y=..density..))+ 
  theme_bw() + 
...
# save plot for mutlipanel figure
saveRDS(B1, file="../plots/B1bw.rds")
```
