...
public website (per email with Ethan Ford).
...
if (!file.exists(bfile)){
  files <- list.files(datdir, pattern = ".CpG.txt", full.names = TRUE)
...
```{r, meta}
if(ncol(pData(bs))==0){
  sampleNames(bs) <- unlist(sapply(strsplit(sampleNames(bs), "//"), 
                                   function(x) x[[2]]))
  ...
  pdat <- meta[meta$basefile %in% sampleNames(bs),] %>%
    mutate(condition = ifelse(grepl("Vector", source_name_ch1), 
                              "Control", "Methylated"),
...
plotEmpiricalDistribution(bs[idx, pData(bs)$condition=="Methylated"], 
                          testCovariate = "dox",
                          bySample = TRUE,
                          type = "Cov")
```

Plots of beta values by sample:
...
Next we have to filter the loci by coverage (remove those with zero coverage
...
regions <- regions[order(regions$qval, seqnames(regions)),]
...
plotDMRs(bs, regions=tail(regions[regions$qval < fdrlevel.dmr,]), testCovariate="condition")
dev.off()
```
...
```{r, compare}
dssfile <- paste0(datdir, "170506-2.txt")
...
plot(density(regions[regions$qval < fdrlevel.dmr, ]$meanDiff),
     main = "Smoothed density of meanDiff",
     xlab = "Mean proportion difference in methylation",
     col="blue")
lines(density(dmrs.dss$meanDiff))
...
# collapse over samples of the same dox/condition (don't print for now)
p <- ggplot(exp, aes(x=count+1, group=interaction(dox, condition), color=condition)) +
  geom_density(adjust=0.75, aes(linetype=dox)) +
  scale_x_continuous(trans="log2") +
  theme_bw() + 
  ggtitle("Smoothed density of raw counts")

p <- ggplot(exp, aes(x=normcount+1, group=interaction(dox, condition), color=condition)) +
  geom_density(adjust=0.75, aes(linetype=dox)) +
  scale_x_continuous(trans="log2") +
  theme_bw() + 
  ggtitle("Smoothed density of normalized counts")
...
ggplot(dmrsa, aes(x = delta_mC, y = log2FoldChange)) +
  ...
  #geom_smooth(data=dmrsa %>% filter(sig==TRUE)) +
  geom_smooth() +
...
```{r}
de_dn <- sum(dmrsc$sig & dmrsc$log2FoldChange < 0, na.rm=TRUE)
de_up <- sum(dmrsc$sig & dmrsc$log2FoldChange > 0, na.rm=TRUE)
```
